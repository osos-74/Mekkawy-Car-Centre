...
return sequelize.transaction(async (transaction) => {  
  ...
  async deleteQuotation(id: number) {
    ...
    const lines = await quotationLineRepository.findByQuotationId(existing.quotationId, transaction);
...
private async recalculateTotals(
    ...
    const quotationLines =
        await quotationLineRepository.findByQuotationId(
            quotationId,
            transaction
        );
...
async getItem(
    ...
    if (dto.partId) {
        const part = await partService.getPartById(
            dto.partId,
            transaction
        );
        if(!part) {
  ...
  async addLine(dto: CreateQuotationLineDto) {
    ...
    return sequelize.transaction(async (transaction) => {
        ...
        // 1. Find quotation
        const quotation = await this.getQuotationById(dto.quotationId,transaction);
       ...
       const newQuantity =
      Number(existingLine.quantity) + dto.quantity;
          console.log("new quantity:", existingLine.quantity);
          existingLine.lineTotal = existingLine.quantity * itemData.unitPrice - discount;
