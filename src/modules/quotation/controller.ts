...
class QuotationController {
  ...
  getQuotations = asyncHandler(async (req: Request, res: Response) => {
    ...
    res
      .status(200)
      .json({ message: "Quotations retrieved successfully", quotations });
  });
  ...
  update = asyncHandler(async (req: Request, res: Response) => {
    const quotation = await quotationService.updateInfo(
  ...
  getQuotationData = asyncHandler(async (req: Request, res: Response) => {
    ...
    res
      .status(200)
      .json({ message: "Quotation PDF Data Retrieved", quotationPdfData });
  });

  approveQuotation = asyncHandler(async (req: Request, res: Response) => {
    await quotationService.approveQuotation(Number(req.params.id));
    res.status(200).json({ message: "Quotation Approved" });
  });
}
